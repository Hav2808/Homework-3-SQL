...
SELECT c.name, p.name, t.name FROM track t
JOIN collection c ON c.id = t.id
JOIN performers p ON p.id = c.id
WHERE p.name = 'Юность'
GROUP BY p.name, t.name, c.name
...
SELECT t.name, t.id, COUNT(c.id) FROM track t
LEFT JOIN collection_track ct ON ct.track_id = t.id
LEFT JOIN collection c ON c.id = ct.collection_id
GROUP BY t.name, t.id, c.name
HAVING COUNT(c.id) = 0
